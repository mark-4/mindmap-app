...
class MindMapView(QGraphicsView):
    ...
    def add_node(self, label: str = "ノード", pos: QPointF | None = None) -> NodeItem:
    ...
    def _calculate_smart_position(self, parent_node: 'NodeItem') -> QPointF:
        """親ノードから適切な位置に子ノードを配置（既存ノード間に割り込み配置）"""
    ...
    def _move_obstructing_nodes_down(self, new_child_pos: QPointF, all_nodes: list['NodeItem'], parent_node: 'NodeItem'):
        """新しい子ノードの位置で他のノードが邪魔になる場合、必ず下に移動して割り込ませる"""
        node_width = 128
        node_height = 80
        min_spacing = 20
        ...
        # 新しい子ノードより下にある全てのノードを下に移動
        self._move_all_nodes_below_down(nodes_to_move_down, new_child_pos, node_height, min_spacing)
    ...
    def _move_all_nodes_below_down(self, nodes_to_move: list['NodeItem'], new_child_pos: QPointF, 
                                  node_height: float, min_spacing: float):
        """新しい子ノードより下にある全てのノードを下に移動"""
        if not nodes_to_move:
            return
        
        # 新しい子ノードの下に配置する位置を計算
        target_y = new_child_pos.y() + node_height + min_spacing
        
        # 各ノードを順次下に移動
        current_bottom_y = target_y
        
        for node in nodes_to_move:
            current_pos = node.pos()
            new_y = current_bottom_y + node_height / 2
            ...
            # 次のノードの位置を更新
            current_bottom_y = new_y + node_height / 2 + min_spacing

    def _analyze_obstructing_hierarchies(self, obstructing_nodes: list['NodeItem'], all_nodes: list['NodeItem']) -> dict:
        """邪魔になるノードの階層構造を分析"""
        hierarchies = {}
        ...
        for node in obstructing_nodes:
            ...
            # ルートノードを特定
            root_node = self._find_root_node(node, all_nodes)
            
            # 階層構造に追加
            if level not in hierarchies:
                hierarchies[level] = {}
            if root_node not in hierarchies[level]:
                hierarchies[level][root_node] = []
            hierarchies[level][root_node].append(node)
        
        return hierarchies

    def _find_root_node(self, node: 'NodeItem', all_nodes: list['NodeItem']) -> 'NodeItem':
        """ノードのルートノードを特定"""
        # ノードの親を辿ってルートノードを見つける
        current = node
        visited = set()
...
class AddNodeCommand(QUndoCommand):
    def __init__(self, view: MindMapView, label: str = "新規ノード", parent_node: NodeItem | None = None, pos: QPointF | None = None):
    ...
    def redo(self):
        if self.node is None:
            self.node = self.view.add_node(self.label, self.pos)
        else:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        print(f"Translucent background supported: {self.testAttribute(Qt.WA_TranslucentBackground)}")
        ...
        def on_add_node():
            ...
            if self.undo_stack is not None:
                self.undo_stack.push(AddNodeCommand(self.view, "新規ノード", parent_node, new_pos))
            else:
                new_node = self.view.add_node("新規ノード", new_pos)
                if parent_node is not None:
